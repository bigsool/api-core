...
class Helper extends BasicHelper {
     ...
     * @throws \Core\Error\FormattedError
     */
    public function getNewAuthToken (array $authToken) {

        if (!isset($authToken['login'])) {
            throw ApplicationContext::getInstance()->getErrorManager()
                                    ->getFormattedError(ERROR_PERMISSION_DENIED); // we may have a better error code
        }
    ...
    public function checkAuthToken ($authToken) {
        ...
        if ($this->generateAuthToken($login, $expiration, $password, $type) != $authToken) {
            throw $errorManager->getFormattedError(ERROR_PERMISSION_DENIED); // we may have a better error code
        }
