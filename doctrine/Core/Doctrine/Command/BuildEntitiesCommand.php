...
class BuildEntitiesCommand extends Command {
    ...
    protected function execute (InputInterface $input, OutputInterface $output) {
        ...
        foreach ($products as $product) {
            ...
            $ymls = array_merge_recursive($ymls, $newXmls);
        ...
        try {
            ...
            $prevProgress = 0;
            $i = 0;
            foreach ($ymls as $modelName => $modules) {
                $models[$modelName] =
                    $this->generateModel($input, $output, $modelName, $modules, $root, $finalProduct, $cmf, $em);
                $this->setCurrentProgression(20 + (++$i / count($ymls) * 40));
            }
    ...
    protected function fetchXmls (InputInterface $input, OutputInterface $output, $basePath, array $modules,
                                  $addExtention, $currProductNb, $nbProduct) {
        ...
        foreach ($modules as $module) {
            ...
            foreach ($xmlFiles as $xmlFile) {
                ...
                if (!isset($xmls[$modelName])) {
                    $xmls[$modelName] = array();
                }
                ...
                && $output->writeln(sprintf("The file '<info>%s</info>' is loaded.", realpath($path . '/' . $xmlFile)));

                $isExtenstion = $dom->documentElement->tagName == 'extention';
                if (!$addExtention && $isExtenstion) {
                    $input->getOption('verbose')
                    && $output->writeln(sprintf("The file '<info>%s</info>' is ignored.",
                                                realpath($path . '/' . $xmlFile)));
                    continue;
                }
            ...
            $this->setCurrentProgression(5 + ($currProductNb / $nbProduct * 10) + (1 * (++$j / count($modules))
                                                                                   / $nbProduct * 10));
    ...
    protected function generateProxy (InputInterface $input, OutputInterface $output, $modelName, $moduleName, $root,
                                      $finalProduct, EntityManager $em) {
        ...
        if (!file_exists($proxyPath)) {
            throw new \InvalidArgumentException(
                sprintf("The proxy '<info>%s</info>' seems to not be created because the file '<info>%s</info>' was not found.",
                        $modelName, $proxyPath)
            );
        }

        $input->getOption('verbose')
        && $output->writeln(sprintf("The proxy '<info>%s</info>' was generated.", $modelName));

    }

    protected function generateModel (InputInterface $input, OutputInterface $output, $modelName, array $modules, $root,
                                      $finalProduct, \Doctrine\ORM\Tools\DisconnectedClassMetadataFactory $cmf,
                                      EntityManager $em) {
        ...
        $input->getOption('verbose')
        && $output->write(sprintf("Merging of the model '<info>%s</info>' ...", $modelName));
                ...
                if ($xml->documentElement->tagName != 'extention') {
                    if (!$moduleName) {
                        ...
                        if ($moduleName != $module) {
                            throw new \InvalidArgumentException(
                                sprintf("The moduleName '<info>%s</info>' is different from the module '<info>%s</info>'.",
                                        $moduleName, $module)
                            );
                        }
                    }
        ...
        $input->getOption('verbose')
        && $output->write(sprintf("Creation of the file '<info>%s</info>' ... ", $modelFilename));
        ...
        if (!file_exists($modelPath)) {
            throw new \InvalidArgumentException(
                sprintf("The model '<info>%s</info>' seems to not be created because the file '<info>%s</info>' was not found.",
                        $modelName, $modelPath)
            );
        }

        $input->getOption('verbose')
        && $output->writeln(sprintf("The model '<info>%s</info>' was generated.", $modelName));
