...
ErrorManager::addDefinedError(new Error(ERR_SERVICE_NOT_FOUND, 'service introuvable', 'service not found',
                                        ERR_REQUEST_INVALID));

define('ERR_METHOD_NOT_FOUND', 4);
ErrorManager::addDefinedError(new Error(ERR_METHOD_NOT_FOUND, 'méthode introuvable', 'method not found',
                                        ERR_REQUEST_INVALID));
...
ErrorManager::addDefinedError(new Error(ERR_API_UNAVAILABLE, 'service temporairement indisponible',
                                        'service temporarily unavailable', ERR_INTERNAL_ERROR));

define('ERR_DB_UNKNOWN_ERROR', 10);
ErrorManager::addDefinedError(new Error(ERR_DB_UNKNOWN_ERROR, 'erreur inconnue lié à la base de données',
                                        'unknown database error', ERR_INTERNAL_ERROR));

define('ERR_DB_CANNOT_BEGIN_TRANSACTION', 12);
ErrorManager::addDefinedError(new Error(ERR_DB_CANNOT_BEGIN_TRANSACTION, 'impossible de démarrer une transaction',
                                        'database cannot begin transaction', ERR_INTERNAL_ERROR));

define('ERR_DB_CANNOT_COMMIT', 13);
ErrorManager::addDefinedError(new Error(ERR_DB_CANNOT_COMMIT, 'commit impossible', 'database cannot commit',
                                        ERR_INTERNAL_ERROR));

define('ERR_DB_CANNOT_ROLLBACK', 14);
ErrorManager::addDefinedError(new Error(ERR_DB_CANNOT_ROLLBACK, 'rollback impossible', 'database cannot rollback',
                                        ERR_INTERNAL_ERROR));

define('ERR_EMAIL_CANNOT_BE_SENT', 15);
ErrorManager::addDefinedError(new Error(ERR_EMAIL_CANNOT_BE_SENT, "erreur lors de l'envoi du mail",
                                        'email cannot be sent', ERR_INTERNAL_ERROR));

define('ERR_UNKNOWN_AMAZON_ERROR', 16);
ErrorManager::addDefinedError(new Error(ERR_UNKNOWN_AMAZON_ERROR, "erreur amazon inconnue", 'unknown amazon error',
                                        ERR_INTERNAL_ERROR));
